...
class My_Theme_Options {
	...
	public function display_page() {
		...
		echo 'var wrapped = $(".wrap h3").wrap("<div class=\"ui-tabs-panel\">");
            ...
            $.getScript("'.get_template_directory_uri().'/athena/lightbox.js", function(){

            });

			// Hide URL Box if Social Icon is not being displayed.
			$(".hideNextRow").change(function() {
			    if (!$(this).is(":checked"))
			...
			$( document ).ready(function() {
			    jQuery(".hide").each(function() {
                    if (!jQuery(this).parents("tr").prev("tr").children("td").children("input").is(":checked"))
	...
	public function display_setting( $args = array() ) {
		...
		switch ( $type ) {
            ...
            case 'url':
                echo '<div field_value="'.$value.'" class="url_box"><input class="submit_url' . $field_class . '" type="text" id="' . $id . '" name="mytheme_options[' . $id . ']" placeholder="' . $std . '" value="' . esc_attr( $options[$id] ) . '" />';
                echo '<br /><input class="upload_image_trigger" value="Upload" type="button"/>';

                if ( $desc != '' )
                    echo '<span class="description">' . $desc . '</span></div>';
